...
def summarize(equity: pd.Series, pnl: pd.Series):
    ...
    sharpe = float((ret.mean()/ (ret.std(ddof=1)+1e-12))*np.sqrt(252*6.5*6)) if len(ret)>2 else 0.0  # 10m bars ~ 6 per hour

    dd = (eq/eq.cummax() - 1.0).min() if len(eq)>1 else 0.0
    calmar = float((eq.iloc[-1]/eq.iloc[0]-1.0)/abs(dd)) if dd<0 else float('inf')

    # trade frequency proxy: changes in position size
    freq = float((pnl != 0).astype(int).rolling(144).sum().mean())  # ~10 days window
