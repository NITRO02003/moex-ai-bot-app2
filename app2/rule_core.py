...
class RuleBtParams:
    ...
    max_daily_dd: float = 0.0      # максимальная дневная просадка (0 = без лимита)
    max_total_dd: float = 0.0      # максимальная общая просадка от пика (0 = без лимита)
    close_on_regime_exit: bool = False  # закрывать позицию при выходе бара из режима
    min_hold_bars: int = 0         # минимальное число баров в сделке до выхода по сигналу
    cooldown_bars: int = 0         # число баров-кулдаун после закрытия сделки
    max_trades_per_day: Optional[int] = None  # лимит числа сделок в день (None = без лимита)
...
def _compute_metrics(equity: pd.Series, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Делает расчёт устойчивым к NaN/inf в equity и pnl.
    """
    metrics: Dict[str, Any] = {}

    # Если вообще нет кривой — возвращаем нули
    ...
    # Очистка equity от NaN/inf, чтобы не получать NaN в total_return
    eq = pd.to_numeric(equity, errors="coerce")
    eq = eq.replace([np.inf, -np.inf], np.nan)
    eq_clean = eq.dropna()

    if eq_clean.empty:
        # Если после очистки ничего не осталось — считаем, что доходность 0,
        # но метрики по сделкам считаем по trades.
        total_return = 0.0
        max_dd = 0.0
        vol = 0.0
        sharpe = 0.0
    else:
        equity0 = float(eq_clean.iloc[0])
        equity_end = float(eq_clean.iloc[-1])
        total_return = equity_end / equity0 - 1.0 if equity0 > 0 else 0.0

        running_max = eq_clean.cummax()
        dd = eq_clean / running_max - 1.0
        max_dd = float(dd.min()) if not dd.empty else 0.0

        # доходности по барам
        rets = eq_clean.pct_change(fill_method=None).fillna(0.0)
    ...
    # по сделкам
    trade_count = len(trades)
    raw_pnls = [t.get("pnl_abs", 0.0) for t in trades] if trades else []
    # фильтруем NaN/inf из pnl
    pnl_list = []
    for p in raw_pnls:
        try:
            v = float(p)
...
def run_rule_symbol(
    ...
    # нормализуем regime_mask под индекс df
    if regime_mask is not None:
        ...
        use_regime_mask = True
    else:
        use_regime_mask = False

    close = df["close"].astype(float)
    signal = df["signal"].fillna(0).astype(float)

    equity_base = float(equity0)
    equity_curve: List[float] = []
    equity_times: List[pd.Timestamp] = []
    peak_equity = equity_base
    drawdowns: List[float] = []

    # риск-менеджмент: глобальная и дневная просадка
    current_date = None
    day_peak_equity = equity_base
    halted_today = False
    hit_max_total_dd = False
    hit_max_daily_dd_days = 0
    ...
    regime_exit_trades = 0

    for i in range(len(df)):
        t = dt.iloc[i]
        # дневное состояние: смена даты, сброс лимитов по дню
        cur_day = t.date()
        if current_date is None or cur_day != current_date:
            current_date = cur_day
            day_peak_equity = equity_base
            halted_today = False

        price = float(close.iloc[i])
        sig = int(np.sign(signal.iloc[i]))
        atr_i = float(atr.iloc[i]) if not np.isnan(atr.iloc[i]) else None

        # ---- проверяем закрытие текущей позиции ----
        if position_dir != 0:
            exit_reason = None

            # выход по смене режима (если включено и есть маска)
            if (
                params.close_on_regime_exit
                and use_regime_mask
                and not bool(regime_mask.iloc[i])
            ):
                exit_reason = "regime_exit"

            # стоп/тейк по ATR
            if exit_reason is None and atr_i is not None and atr_i > 0:
                move = (price - entry_price) * position_dir
        ...
        if position_dir == 0 and sig != 0:
            # после срабатывания лимитов просадки новые сделки не открываем
            if hit_max_total_dd or halted_today:
                qty = 0.0
            else:
        ...
        # дневной пик equity и дневная просадка
        if day_peak_equity is None:
            day_peak_equity = equity_bar
        else:
            if equity_bar > day_peak_equity:
                day_peak_equity = equity_bar
        if day_peak_equity > 0:
            day_dd = equity_bar / day_peak_equity - 1.0
        else:
            day_dd = 0.0

        # фиксация факта срабатывания лимитов просадки (блокируют новые входы)
        if (not hit_max_total_dd) and params.max_total_dd > 0 and dd <= -params.max_total_dd:
            hit_max_total_dd = True
        if (not halted_today) and params.max_daily_dd > 0 and day_dd <= -params.max_daily_dd:
            halted_today = True
            hit_max_daily_dd_days += 1
    ...
    # добавляем в метрики информацию о лимитах и их срабатывании
    metrics["max_total_dd_limit"] = params.max_total_dd
    metrics["max_daily_dd_limit"] = params.max_daily_dd
    metrics["hit_max_total_dd"] = hit_max_total_dd
    metrics["hit_max_daily_dd_days"] = hit_max_daily_dd_days
    metrics["regime_exit_trades"] = regime_exit_trades
    ...
    if collect_bar_stats:
        result["bar_stats"] = pd.DataFrame(bar_rows)
