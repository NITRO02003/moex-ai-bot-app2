...
def cmd_train(a):
    ...
    payload = {"timestamp": _now_iso(), "trained": info}
...
def cmd_validate(a):
    ...
    payload = {"timestamp": _now_iso(), "results": out}
...
def cmd_calibrate(a):
        ...
        if a.asymmetric:
            ...
            for th in grid:
                if a.objective == "profit":
                    ...
                    score = float(pd.Series(p.values).rank().corr(pd.Series(ybin)))
                if score > best_score:
                    best_score = score; best = {"threshold": float(th)}
    ...
    payload = {"timestamp": _now_iso(), "calibration": res}
    with open(path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(str(path))
...
def cmd_backtest(a):
    ...
    calib_wrap = _load_calibration() if getattr(a, "use_calibrated", False) else {}
    calib = calib_wrap.get("calibration", calib_wrap) if isinstance(calib_wrap, dict) else {}
    summary = {}
    for s in a.symbols:
        ...
        th_long = sym_cal.get("th_long"); th_short = sym_cal.get("th_short")
        res = _B.run_symbol(prices, bundle, rp, bt, equity0=1_000_000.0,
                            threshold=th, th_long=th_long, th_short=th_short)
        _B.save_report(s, res)
        summary[s] = res["metrics"]
    path = Path(a.out) if getattr(a, "out", None) else (REPORTS_DIR / "backtest_summary.json")
    payload = {"timestamp": _now_iso(), "results": summary}
    with open(path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(str(path))
...
def cmd_regime_backtest(a):
    ...
    for s in a.symbols:
        prices = _D.load_csv(s)
        if prices is None or getattr(prices, "empty", False):
            print(f"[warn] no price data for {symbol}, skip")
            continue
        sc = calib.get(s, {})
...
def cmd_diag(a):
    ...
    payload = {"timestamp": _now_iso(), "results": info}
    with open(path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(str(path))
...
def cmd_sweep(a):
    _ensure_dirs()
    from .run_both import SweepConfig, run_single_param_sweeps
    cfg = SweepConfig(symbols=a.symbols, horizon=a.horizon, n_jobs=a.n_jobs)
    res = run_single_param_sweeps(cfg, out_csv=a.out_csv, top_k=a.top_k, out_best_json=a.out_best)
    print(json.dumps({"timestamp": _now_iso(), **res}, ensure_ascii=False))
...
def main():
    ...
    p_s.add_argument("--symbols", nargs="+", required=True)
    p_s.add_argument("--horizon", type=int, default=1)
    p_s.add_argument("--n-jobs", dest="n_jobs", type=int, default=max(1, (os.cpu_count() or 2) - 1))
    p_s.add_argument("--out-csv", dest="out_csv", default=str(REPORTS_DIR / "sweep_results.csv"))
