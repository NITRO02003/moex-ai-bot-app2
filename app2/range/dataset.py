...
def make_entry_candidates(
    ...
    """Build Dataset A from candidate entry bars (zone touches).

    One row per candidate entry, labeled by forward horizon return
    or risk-aware conditions (MFE/MAE).
    """
    ...
    required_cols = {"high", "low", "close", "v3_L", "v3_U"}
    if not required_cols.issubset(fdf.columns):
        return pd.DataFrame()

    high = pd.to_numeric(fdf["high"], errors="coerce")
    low = pd.to_numeric(fdf["low"], errors="coerce")
    close = pd.to_numeric(fdf["close"], errors="coerce")
    v3_L = pd.to_numeric(fdf["v3_L"], errors="coerce")
    v3_U = pd.to_numeric(fdf["v3_U"], errors="coerce")

    H = (v3_U - v3_L).replace(0, np.nan)
    shadow = H * float(shadow_pct)
    alpha = H * float(entry_zone_alpha)

    long_zone_low = v3_L - shadow
    long_zone_high = v3_L + alpha
    short_zone_low = v3_U - alpha
    short_zone_high = v3_U + shadow

    is_in_zone_long = (low <= long_zone_high) & (high >= long_zone_low)
    is_in_zone_short = (high >= short_zone_low) & (low <= short_zone_high)

    side = pd.Series(0, index=fdf.index, dtype=int)
    side = side.where(~is_in_zone_long, 1)
    side = side.where(~is_in_zone_short, -1)
    side[(is_in_zone_long & is_in_zone_short)] = 0

    entry_price = None
    if "open" in fdf.columns:
        entry_price = pd.to_numeric(fdf["open"], errors="coerce").shift(-1)
    else:
        entry_price = close.shift(-1)

    entry_dt = fdf["datetime"].shift(-1)

    horizon = int(max(horizon_bars, 1))
    future_close = close.shift(-horizon)
    shifted_high = high.shift(-1)
    shifted_low = low.shift(-1)
    forward_high = shifted_high.rolling(window=horizon, min_periods=horizon).max().shift(-(horizon - 1))
    forward_low = shifted_low.rolling(window=horizon, min_periods=horizon).min().shift(-(horizon - 1))

    entry_ret = (future_close - entry_price) / entry_price.replace(0, np.nan)
    entry_mfe = (forward_high - entry_price) / entry_price.replace(0, np.nan)
    entry_mae = (forward_low - entry_price) / entry_price.replace(0, np.nan)

    # adjust for short candidates
    short_mask = side == -1
    entry_ret = entry_ret.where(~short_mask, -entry_ret)
    entry_mfe = entry_mfe.where(~short_mask, (entry_price - forward_low) / entry_price.replace(0, np.nan))
    entry_mae = entry_mae.where(~short_mask, (entry_price - forward_high) / entry_price.replace(0, np.nan))

    label_mode = str(label_mode or "ret").lower()
    if label_mode == "ret":
        y_entry = (entry_ret >= float(return_threshold))
    elif label_mode == "mfe_mae":
        y_entry = (entry_mfe >= float(mfe_threshold)) & (entry_mae >= -float(mae_threshold))
    elif label_mode == "ret_mae":
        y_entry = (entry_ret >= float(return_threshold)) & (entry_mae >= -float(mae_threshold))
    elif label_mode == "quantile":
        q = float(quantile)
        q = max(0.0, min(0.49, q))
        if drop_middle:
            low_thr = entry_ret.quantile(q)
            high_thr = entry_ret.quantile(1.0 - q)
            y_entry = pd.Series(np.nan, index=fdf.index, dtype=float)
            y_entry = y_entry.where(~(entry_ret <= low_thr), 0.0)
            y_entry = y_entry.where(~(entry_ret >= high_thr), 1.0)
        else:
            thr = entry_ret.quantile(1.0 - q)
            y_entry = (entry_ret >= thr)
    else:
        raise ValueError(f"Unknown entry label_mode: {label_mode}")
    y_entry = pd.to_numeric(y_entry, errors="coerce")

    mask = (side != 0) & entry_price.notna() & entry_dt.notna() & entry_ret.notna()
    if drop_middle and label_mode == "quantile":
        mask = mask & y_entry.notna()
    candidates = fdf.loc[mask].copy()
    candidates["signal_dt"] = candidates["datetime"]
    candidates["entry_dt"] = entry_dt.loc[mask]
    candidates["entry_price"] = entry_price.loc[mask]
    candidates["side"] = side.loc[mask]
    candidates["entry_horizon_bars"] = horizon
    candidates["entry_ret"] = entry_ret.loc[mask]
    candidates["entry_mfe"] = entry_mfe.loc[mask]
    candidates["entry_mae"] = entry_mae.loc[mask]
    candidates["y_entry"] = y_entry.loc[mask].astype(int)
    candidates["interval"] = interval

    # Drop raw datetime column to avoid ambiguity
    candidates = candidates.drop(columns=["datetime"], errors="ignore")
