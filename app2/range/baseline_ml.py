...
def _prepare_features(
    ...
    if include:
        keep = [col for col in include if col in num.columns]
        num = num[keep].copy()
    for col in exclude:
...
def _split_by_symbol_time(
    ...
    if symbol_col not in df.columns:
        return _split_by_time(df, time_col, test_size)

    work = df.copy()
    work[time_col] = pd.to_datetime(work[time_col], errors="coerce")
    work[symbol_col] = work[symbol_col].fillna("__missing__").astype(str)
    ...
    train_df = pd.concat(train_parts, ignore_index=True) if train_parts else work.iloc[:0].copy()
    test_df = pd.concat(test_parts, ignore_index=True) if test_parts else work.iloc[:0].copy()
    return train_df, test_df


def _split_by_symbol_trade(
    df: pd.DataFrame,
    symbol_col: str,
    trade_col: str,
    time_col: str,
    ...
    if symbol_col not in df.columns:
        return _split_by_trade(df, trade_col, time_col, test_size)
    ...
    test_parts: List[pd.DataFrame] = []
    for _, group in work.groupby(symbol_col):
        trades = group[[trade_col, time_col]].drop_duplicates()
        trades = trades.sort_values(time_col)
        n = len(trades)
        if n == 0:
            continue
        ...
        split_idx = max(1, min(n - 1, split_idx))
        train_trades = set(trades.iloc[:split_idx][trade_col].tolist())
        train_parts.append(group[group[trade_col].isin(train_trades)])
        test_parts.append(group[~group[trade_col].isin(train_trades)])
...
def _run_entry_baseline(
    ...
    if split_mode == "per_symbol":
        train_df, test_df = _split_by_symbol_time(df, "symbol", "entry_dt", test_size)
    else:
    ...
    x_train_df = _prepare_features(train_df, exclude, include=include)
    x_test_df = _prepare_features(test_df, exclude, include=include)
...
def _run_intrade_baseline(
    ...
    if split_mode == "per_symbol":
        train_df, test_df = _split_by_symbol_trade(df, "symbol", "trade_uid", "entry_dt", test_size)
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=["all", "compact"],
        default="all",
        help="Feature set selection (compact excludes proxy features by design).",
    )
    parser.add_argument(
        "--exclude-entry",
        type=str,
