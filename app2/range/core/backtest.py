...
def _run_trades_from_signals(
    ...
    circuit_breaker_time: Optional[pd.Timestamp] = None
    ...
    for i, (ts, sig) in enumerate(signals.items()):
        ...
        if position_side != 0:
            ...
            if need_close:
                # Determine exit reason (analytics only)
                exit_reason = "unknown"
                if hit_sl:
                    exit_reason = "sl"
                elif hit_tp:
    ...
    metrics = {
        ...
        "equity0": equity0,
        "equity_final": equity,
        "circuit_breaker_hit": bool(stopped_by_circuit_breaker),
        "circuit_breaker_time": (str(circuit_breaker_time) if circuit_breaker_time is not None else None),
    }
