...
def _run_symbol_for_combo(
    args: tuple[str, str, float, RangeV3Params],
) -> tuple[str, Dict[str, Any] | None, pd.DataFrame]:
    symbol, interval, equity0, params = args
    try:
...
def run_sweep(
    ...
    for overrides in _iter_grid(grid):
        ...
        tasks = [(symbol, interval, equity0, params) for symbol in symbols]
        for symbol, metrics, trades_df in parallel_map(tasks, _run_symbol_for_combo, n_jobs=n_jobs):
            if metrics is None:
                continue
