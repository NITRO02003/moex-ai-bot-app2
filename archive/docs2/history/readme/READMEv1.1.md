# READMEv1.1 - MOEX APP2+ / docs2 source of truth

Файл: `docs2/READMEv1.1.md`

Эта версия исправляет ошибку v1.0:
- новый документ не должен переписывать старую историю с нуля;
- новая версия должна дополнять прежнее содержание, а не заменять его;
- исключения допускаются только по явному согласованию.

## 0. Правила ведения версий docs2

1. Любое изменение документа - это новая версия файла.
2. Смысл и хронология предыдущих версий сохраняются.
3. Новая версия дополняет и структурирует прошлое содержание, а не обнуляет его.
4. При миграции чата новый чат обязан сначала читать:
   - `docs2/READMEv1.1.md`
   - `docs2/DOC_INDEX_v1.1.md`
   - `docs2/operations/MIGRATION_CONTEXT_v1.1.md`
   - `docs2/CONTRACTv1.1.md`
   - `docs2/model_plan_v1.1.md`

## 1. Текущая рамка проекта

На текущем срезе:
- core является главным контуром развития;
- legacy остаётся только как reference / sanity-check;
...
- Dataset A truth policy будет разведена на:
...
- `agent_hands.py` существует как bridge к локальной модели и сейчас должен трактоваться как read-only;
- документы `docs2` являются новым source of truth для миграции и последующего планирования.

## 3. Сохранённая история README без потери смысла

Ниже verbatim-содержимое последних согласованных README, чтобы не терять хронологию.
