# model_plan_v1.1 - сводный план развития модели и системы

Файл: `docs2/model_plan_v1.1.md`

Эта версия исправляет ошибку v1.0:
- новый plan не должен терять содержимое прошлых model_plan;
- новая структура должна сохранять последовательность уже согласованных фаз.
...
- новые предложения внешних аналитиков включаются в план, если с ними согласились, но их приоритет определяется стадией разработки;
- Dataset A truth policy фиксируется как:
  - `Dataset A_research = candidates`
  - `Dataset A_policy = trades`.

## 1. Что должен знать новый чат о плане

- нужно опираться не только на ближайшие задачи, но и на целевую картину модулей к поздним фазам;
- roadmap должен учитывать не только текущие модули, но и будущую архитектуру до поздних фаз model_plan;
- docs2 должна сохранять хронологию и смысл старых планов, а не переписывать их как "с чистого листа".

## 2. Сохранённые model_plan без потери смысла
