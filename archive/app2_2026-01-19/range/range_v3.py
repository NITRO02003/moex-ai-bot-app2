...
class RangeV3Params:
    ...
    def sl_pct(self) -> float:
        """Stop-loss distance as fraction of price (e.g. 0.01 = 1%)."""
        return float(self.cfg.get("sl_pct", 0.01))

    @property
    def tp_pct(self) -> float:
        """Take-profit distance as fraction of price (e.g. 0.015 = 1.5%)."""
        return float(self.cfg.get("tp_pct", 0.015))

    @property
    def slope_k(self) -> float:
        """Threshold for relative MA slope in flat regime filter."""
        return float(self.cfg.get("slope_k", 0.0005))

    @property
    def atr_window(self) -> int:
        """ATR window in bars for rolling regime filter."""
        return int(self.cfg.get("atr_window", self.range_window_bars))

    @property
    def atr_pct_min(self) -> float:
        """Minimum ATR/price ratio for regime filter."""
        return float(self.cfg.get("atr_pct_min", 0.001))

    @property
    def atr_pct_max(self) -> float:
        """Maximum ATR/price ratio for regime filter."""
        return float(self.cfg.get("atr_pct_max", 0.02))

    @property
    def atr_min_valid_frac(self) -> float:
        """Fraction of valid bars in ATR window (0.0–1.0)."""
        return float(self.cfg.get("atr_min_valid_frac", 0.4))

    @property
    def atr_min_valid_bars(self) -> int:
        """Minimum number of valid bars in ATR window."""
        return int(self.cfg.get("atr_min_valid_bars", 10))
...
def detect_range_segments_v3_with_debug(
    df: pd.DataFrame, params: RangeV3Params
) -> Tuple[List[RangeSegmentV3], Dict[str, Any]]:
    """Detect segments with rejection reasons for debug tooling."""
    ...
    debug_info["segments_total"] = len(segments_idx)
    ...
    def _bump(reason: str) -> None:
        debug_info["segments_rejected"] += 1
        debug_info["reasons_count"][reason] = debug_info["reasons_count"].get(reason, 0) + 1
    ...
    for start_idx, end_idx in segments_idx:
        ...
        H_raw = U_raw - L_raw
        if not math.isfinite(H_raw) or H_raw <= 0:
            _bump("invalid_height")
            continue

        mid_price = float(seg_close.mean())
        if not math.isfinite(mid_price) or mid_price <= 0:
            _bump("invalid_mid_price")
            continue

        height_pct = H_raw / mid_price
        if height_pct < params.min_range_height_pct or height_pct > params.max_range_height_pct:
            _bump("height_out_of_bounds")
            continue
