...
def _calibrate_asym_profit(symbol: str, horizon: int = 1) -> Tuple[float, float]:
    ...
    close = df["close"].astype(float).reindex(X.index).ffill().fillna(0.0)
...
def _bt_volq_worker(args: Tuple[str, int, float, float, float, float]) -> Dict[str, Any]:
    ...
    vol = R.realized_vol(close, 48).reindex(side.index).ffill().fillna(0.0)
    mask = vol >= vol.rolling(200).quantile(q).reindex(side.index).ffill()
