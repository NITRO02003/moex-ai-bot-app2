...
def realized_vol(close: pd.Series, lb: int = 48) -> pd.Series:
    ...
    return ret.replace(0.0, np.nan).ffill().fillna(0.0)
...
def conf_gate(p: pd.Series, close: pd.Series, rp: RiskParams) -> pd.Series:
    ...
    if isinstance(p, pd.Series):
        p = p.astype(float).reindex(close.index).ffill().fillna(0.0)
    else:
        p = pd.Series(float(p), index=close.index)

    vol = realized_vol(close, rp.vol_lb).reindex(close.index).ffill().fillna(0.0)
    q = vol.rolling(200, min_periods=20).quantile(0.5).reindex(close.index).ffill()
    is_volatile = vol >= q
...
def position_size(close: pd.Series, p: Union[float, pd.Series], equity: float, rp: RiskParams) -> pd.Series:
    ...
    """
    close = close.astype(float)
    vol = realized_vol(close, rp.vol_lb).reindex(close.index).replace(0.0, np.nan).ffill()

    if isinstance(p, pd.Series):
        p_series = p.astype(float).reindex(close.index).ffill().fillna(0.0)
    else:
    ...
    denom = (vol * p_series).replace(0.0, np.nan).ffill()
    ...
    return notional.reindex(close.index).ffill().fillna(0.0)
