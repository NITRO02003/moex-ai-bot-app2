...
def signal_and_size(prices: pd.DataFrame, model_bundle, rp: R.RiskParams, equity0: float,
    ...
    p = M.predict_proba(model_bundle, X, close).reindex(prices.index).ffill().fillna(0.0)
    ...
    size = R.position_size(close, p, equity0, rp).reindex(prices.index).ffill().fillna(0.0)
    size = size.where(conf_mask, 0.0)

    if use_filters:
        # 3) Фильтр по волатильности и тренду (EMA 20/60)
        vol = R.realized_vol(close, rp.vol_lb).reindex(prices.index).ffill().fillna(0.0)
        vol_thr = vol.rolling(200, min_periods=20).quantile(0.2).reindex(prices.index).ffill()
        vol_ok = vol >= vol_thr
