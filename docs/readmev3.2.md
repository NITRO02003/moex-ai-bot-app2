...
4. **Self-check кода перед отправкой.**  
   Перед тем как прислать патч с изменёнными файлами, ассистент обязан сделать минимальную самопроверку:
   импорт модулей, базовый прогон ключевых функций на синтетических данных или dry-run, чтобы отловить явные ошибки.

5. **Краткие ответы после правок кода.**  
   После правок в коде текстовые ответы должны быть максимально компактными и по сути:
   короткое описание изменений + ссылка на архив. Развёрнутые объяснения — только по запросу.
