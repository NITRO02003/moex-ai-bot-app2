...
- затем forensic-разбор текущего baseline и вынесение приговора ядру Range V3;
- новые модели, calibration, Regime Engine, sandbox и online не делаются до появления доказанного edge хотя бы в узком сегменте;
- если edge найден, сначала вводится тупой rule-filter на найденном сегменте, и только потом допускается ML-селектор;
- если edge не найден, ветка Range V3 замораживается, а дальнейшая работа переносится на новую стратегическую гипотезу;
...
## 2.1. Приоритетный backlog ближайших доработок

### P_forensic - приговор ядру Range V3
Сначала выполняется не новый ML-блок, а forensic-first цикл по уже существующим артефактам:
- разрезы по символам, причинам выхода, хвостам `pnl_rel`, часу входа, ATR, высоте бокса, slope, положению внутри бокса и длительности удержания;
- обязательная временная проверка найденных сегментов по периодам `early` / `mid` / `late`;
- критерий живого сегмента - не меньше 30-50 сделок и `pf > 1.05`, лучше `> 1.1`, без опоры на 1-2 аномальные сделки;
- до завершения forensic-цикла запрещены новые модели, calibration, Regime Engine и sandbox-движение.


### P0 - критические проблемы доверия к результатам
Сначала закрываются блокеры, из-за которых нельзя доверять offline-результатам:
- привести risk config к единому source-of-truth;
- зажать train/inference parity через artifact contract и validator;
- ввести жёсткую temporal split-проверку для train/test;
- убрать optimistic bias при подборе порога модели.

### P1 - архитектурная расчистка и техдолг
После P0 закрывается техдолг, который уже мешает развитию core:
- убрать дубли сигнальной логики;
- окончательно очистить `core/backtest.py` и вести orchestration напрямую через runner;
- унифицировать rolling-расчёты на стыках дней;
...
До повторного приговора по active core обязательна стабилизация data contract:
- закрепить контракт реального бара для `data/` и `processed/`;
- убрать synthetic empty bars из `processed/`;
- унифицировать core loader вокруг этого контракта;
- ввести sanity-check как обязательный validator data layer;
- только после этого повторно смотреть на forensic core-ветки.

Текущий фокус не в новой надстройке, а в четырёх направлениях:

1. Forensic-отчёт и вынесение приговора ядру Range V3
2. Поиск устойчивого rule-based сегмента, если он вообще существует
3. Проверка гипотезы о том, что прибыль режет exit layer, а не сам вход
4. Жёсткое решение `salvage` или `freeze Range V3`, а не очередная косметическая доработка

## 5. Что прямо не делаем до вердикта по Range V3

- не обучаем новые модели поверх отрицательного baseline;
- не делаем calibration вероятностей;
- не вводим `Regime Engine v1` и adaptive threshold;
- не двигаемся в sandbox / online;
- не плодим новые фичи без forensic-доказательства edge;
- не подменяем исследование "красивыми" офлайн-метриками и массовыми свипами.
...
- локальные агенты допускаются как аналитический слой только в рамках жёстких ограничений и без обхода patch discipline.
